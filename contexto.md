...
Flujo de datos:
Abrir → lee sesión activa de `localStorage` → si vacío, muestra estado vacío
Al cargar datos → `fetchLatestPrices()` automático desde Supabase + `fetchPricesHistory()` + `fetchFXHistory()` + `fetchSnapshots()`
Importar .xls → `handleFileImport()` detecta HTML vs XLSX → parsea → `mergeTransactions()` → `processAndRefresh()` → guarda en localStorage (key de sesión) → `fetchLatestPrices()`
Botón 🔄 Actualizar precios → re-fetchea Supabase manualmente
Botón ⟳ Recalcular (tab Evolución) → reconstruye snapshots históricos → upsert en `portfolio_snapshots`
Borrar sesión → borra localStorage de esa sesión + overrides NRA en Supabase para esa sesión
---
Sistema de sesiones
Concepto
...
Deployar el cambio
El tab Portfolio refleja el cambio inmediatamente (tiempo real)
El tab Evolución muestra datos stale hasta que el usuario presione ⟳ Recalcular
Recordar avisar al usuario que debe recalcular
---
...
Consumir del slot propio primero
Si no alcanza, consumir del otro slot AR
Si todavía no alcanza, transferencia implícita EEUU → AR
...
Letras — ticker contiene `-`
Bonos — ticker está en `BOND_TICKERS`
FCI — tiene txn con `Suscripción FCI` o `Rescate FCI`
Acciones — alphanumeric puro `/^[A-Z0-9]+$/`
Otros — fallback
...
Stale: `Math.max(...years) <= 2020` — pero si `_ledger[ticker].totals.balTotal > 0`, no es stale
Fully amortized: buys + Pago de Amortización ≥ 80% del costo, sin sells
---
...
Reconstruye posiciones EEUU con `buildPortfolio` → `getPriceForDate`
Calcula AR en USD desde `_ledger`: `snap.avgAR_usd × snap.balAR` para cada ticker con `balAR > 0`
Suma cash AR: `bUSD_AR + bARS / getFXForDate(date)`
Suma cauciones en pesos activas a esa fecha: `c.monto / fx` para cada caución con `fechaApertura <= date && (fechaCierre === null || fechaCierre > date)`
Guarda `{ date, ar_ars: 0, ar_usd, us_usd, fx_rate }`
...
Guarda `idx` en cada snapshot → DELETE de todos los snapshots existentes → upsert a Supabase (columna `idx float8` en `portfolio_snapshots`)
